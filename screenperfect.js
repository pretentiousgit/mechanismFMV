...
app.configure(function() {
    ...
    app.use(express.logger(
        '\033[90m:date :method :url :response-time\\ms\033[0m \033[31m:referrer \033[0m'
    ));
...
app.listen(3003, function() {
    ...
    console.log('   app is listening on\033[31m http://' + addr.address + ':' +
        addr.port + '\033[0m');
});
...
io.sockets.on('connection', function(socket) {
    ...
    socket.on('setup event', function(e) {
        ...
        fs.writeFile('tmp/' + nameOfFile + ".json", JSON.stringify(e),
            function(err) {
                if (err) {
                    console.log(err);
                } else {
    ...
    socket.on('listener', function(listener, e) {
        eavesdropper++;
        console.log('eavesdropper ' + eavesdropper +
            ' connected, _hopper is ' + _hopper);
    });
